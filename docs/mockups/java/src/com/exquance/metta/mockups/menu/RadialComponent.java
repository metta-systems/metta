...
abstract class RadialComponent extends Component {
    ...
    public RadialComponent(Sketch p) {
        super(p);
    }
    ...
    // FIXME: static    
    public boolean on_a_ring(float point_x, float point_y,
                    float ring_x, float ring_y,
                    float min_radius, float max_radius) {
        float dx = p.abs(point_x - ring_x);
        float dy = p.abs(point_y - ring_y);
        ...
        if ((dx <= max_radius) && (dy <= max_radius)) {            
             ...
             float x_max_way = max_radius * p.cos(at_angle);
             float y_max_way = max_radius * p.sin(at_angle);
             // get distance to the instersection point 
             // of this angle and ring with min radius
             float x_min_way = min_radius * p.cos(at_angle);
             float y_min_way = min_radius * p.sin(at_angle);             
