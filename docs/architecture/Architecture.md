...
Applications are vertically integrated - they perform most of the management themselves with high adaptivity
and maintain QoS guarantees by removing contention in services usually done by the OS, which often become bottlenecks.
...
queried; these will always be a multiple of the machine's page size. The stretch is a part of protection domain.
...
protections and mappings on the relevant stretch. Domain may grant read/write/execute to another domain, if needed.
...
module dependencies, determine load order and load the TCB modules — these are the most important system modules,
...
important, therefore, to start with a good type system, and [Evers93] presents a good discussion of the issues of
...
software engineering benefits of purely abstract type systems such as that used in Emerald [Raj91], and the
...
integers of various sizes) or constructed (as with records, sequences, etc). The space of concrete types also includes
...
There are few restrictions on how the name space is structured. The model followed is that of [Saltzer79]:
...
be specified. Context can bind names to other contexts, therefore providing a recursive tree-like structure.
...
Runtime type system is a basis for reflectivity support and allows components to introspect their runtime environment,
add new runtime types and resolve arising conflicts.

Together with Sjofn, type system allows service-oriented component combination and relatively easy component adaptation,
similar to Scala language.
...
the Domain Control Block. Kernel uses DCBs to schedule domains' CPU time and other resources.


=== Interfaces ===

Interfaces are defined in an IDL language called Meddle, a compiler named meddler will generate proper stubs from these
IDL declarations. Interface compiler is based on LLVM framework together with the rest of JIT system.
