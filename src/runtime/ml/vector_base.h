...
struct stl_allocation_policy
{
    ...
    {
        return size < capacity ? size : ((capacity + 1) << 1);
    }
};
...
public :
    ...
    /// Returns true if vector container is contains no items. 0(1).
    bool empty() const
    {
        return !m_size;
    }
    ...
    /// Retruns reference to first item in sequence.
    reference front()
    {
        return *begin();
    }
    ...
    /// Retruns reference to last item in sequence.
    reference back()
    {
        return *(end() - 1);
    }
    ...
    /**
     Reallocates memory if nesacery.
    */
